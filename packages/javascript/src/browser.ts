...
export function compressBrowser(code: string, language: Language): BrowserCompressionResult {
    ...
    function trackAbbreviations(before: string, after: string, mapping: Record<string, string>): void {
    ...
    const widgetsOrComponents = language === 'dart'
        ? (abbrevs.widgets || {})
        : (abbrevs.components || {});
    ...
    const percentageSaved = originalLength > 0
